 ...
 */
WebAuthn.prototype.register = async function (publicKey, callback) {
	let publicKeyCredential = Object.assign({}, publicKey)
	publicKeyCredential.user.id = this._bufferDecode(publicKey.user.id)
	publicKeyCredential.challenge = this._bufferDecode(
		this._base64Decode(publicKey.challenge)
	)
	if (publicKey.excludeCredentials) {
		publicKeyCredential.excludeCredentials = this._credentialDecode(
...
WebAuthn.prototype._registerCallback = function (publicKey, callback) {
	let publicKeyCredential = {
		...
		response: {
			...
			clientDataJSON: this._bufferEncode(
				publicKey.response.clientDataJSON
			),
			attestationObject: this._bufferEncode(
 ...
 * @param {function(PublicKeyCredential)} callback  User callback
 */
WebAuthn.prototype.sign = async function (publicKey, callback) {
	let publicKeyCredential = Object.assign({}, publicKey)
	publicKeyCredential.challenge = this._bufferDecode(
		this._base64Decode(publicKey.challenge)
	)
	if (publicKey.allowCredentials) {
		publicKeyCredential.allowCredentials = this._credentialDecode(
...
WebAuthn.prototype._signCallback = function (publicKey, callback) {
	let publicKeyCredential = {
		...
		response: {
			...
			authenticatorData: this._bufferEncode(
